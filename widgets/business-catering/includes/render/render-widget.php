<?php
/**
 * Render logic for Business Catering Widget
 */
...
// Get catering images
$catering_images = $settings['catering_images'];
    ...
    <?php if ($settings['show_title'] === 'yes' && !empty($settings['section_title'])) : ?>
        <header class="promen-catering-title-wrapper">
            ...
            if (function_exists('promen_render_split_title')) {
                ...
                echo promen_render_split_title($this, $title_settings, 'section_title', 'promen-catering');
            } else {
                // Fallback if the function is not available
                $tag = $settings['title_html_tag'] ? $settings['title_html_tag'] : 'h2';
                echo "<{$tag} class='promen-catering-title' id='" . esc_attr($title_id) . "'>{$settings['section_title']}</{$tag}>";
            }
            ?>
        </header>
    <?php endif; ?>
                        ...
                        <div class="swiper-slide" 
                            ...
                            <div class="promen-catering-image-wrapper">
                                <figure class="promen-catering-image elementor-animation-<?php echo esc_attr($settings['image_hover_animation']); ?>" 
                                        role="img" 
                                        aria-label="<?php echo esc_attr($item['title'] ?: sprintf(__('Catering afbeelding %d met informatie over onze cateringdiensten', 'promen-elementor-widgets'), $index + 1)); ?>">
                                    <?php if (!empty($item['image']['url'])) : ?>
                                        <img src="<?php echo esc_url($item['image']['url']); ?>" 
                                             alt="<?php echo esc_attr($item['title'] ?: sprintf(__('Catering image %d', 'promen-elementor-widgets'), $index + 1)); ?>" 
