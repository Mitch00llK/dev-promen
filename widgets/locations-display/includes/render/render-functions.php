<?php
/**
 * Locations Display Render Functions
 *
 * @package Promen
 */
...
class Promen_Locations_Display_Render {
    ...
    public static function render_widget($widget, $settings) {
        ...
        <section class="<?php echo esc_attr($container_class); ?>" 
            ...
            <?php if ('yes' === $settings['show_heading_section']) : ?>
                <header class="locations-heading" id="<?php echo esc_attr($heading_id); ?>">
                    ...
                    echo promen_render_split_title($widget, $settings, 'heading_text', 'locations'); 
                    ?>
            ...
            <div class="locations-grid columns-<?php echo esc_attr($columns); ?>" 
                ...
                if (!empty($settings['locations'])) {
                    foreach ($settings['locations'] as $index => $location) {
                        self::render_location_item($location, $index);
                    }
                }
                ?>
            </div>
        ...
        <?php if ($animation_enabled) : ?>
            <?php self::render_animation_script($widget_id, $animation_type, $animation_delay); ?>
        <?php endif; ?>
        <?php
    }
     ...
     */
    private static function render_location_item($location, $index) {
     ...
     * @param int $animation_delay Animation delay in ms
     */
    private static function render_animation_script($widget_id, $animation_type, $animation_delay) {
        ?>
        <script>
        (function() {
            'use strict';
                    ...
                    if (entry.isIntersecting && !isIntersecting && !animationComplete) {
                        ...
                        gsap.to(animationItems, {
                            ...
                            onComplete: function() {
                                ...
                                gsap.set(animationItems, { clearProps: "all" });
