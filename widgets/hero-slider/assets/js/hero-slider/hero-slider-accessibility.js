    ...
    const HeroSliderAccessibility = {
        ...
        togglePlayPause: function() {
            ...
            if (swiper.autoplay.running) {
                ...
                HeroSliderAccessibility.announceToUser('De automatische diavoorstelling is gepauzeerd');
            } else {
                swiper.autoplay.start();
                HeroSliderAccessibility.updatePlayPauseButton($button, true);
                HeroSliderAccessibility.announceToUser('De automatische diavoorstelling is gestart en wordt nu afgespeeld');
            }
        },
        ...
        updatePlayPauseButton: function($button, isPlaying) {
            ...
            if (isPlaying) {
                ...
                $controlText.text('Pauzeer de automatische diavoorstelling');
                $button.attr('aria-label', 'Klik om de automatische diavoorstelling te pauzeren');
            } else {
                $playIcon.hide();
                $pauseIcon.show();
                $button.attr('aria-pressed', 'false');
                $controlText.text('Start de automatische diavoorstelling');
                $button.attr('aria-label', 'Klik om de automatische diavoorstelling te starten');
            }
        },
            ...
            setTimeout(() => {
                $pagination.find('.swiper-pagination-bullet').each(function(index) {
                    ...
                    $bullet.attr('role', 'tab')
                        .attr('aria-label', `Klik om naar dia ${index + 1} van de diavoorstelling te gaan`)
        ...
        announceSlideChange: function(swiper, $container, trigger = 'auto') {
            ...
            if (title) {
                announcement = `U bekijkt nu dia ${activeIndex + 1} van ${totalSlides} met de titel: ${title}`;
            } else {
                announcement = `U bekijkt nu dia ${activeIndex + 1} van ${totalSlides} in de diavoorstelling`;
            }

            // Add context for keyboard navigation
            if (trigger === 'navigation') {
                announcement = `U heeft genavigeerd naar ${announcement}`;
            }
