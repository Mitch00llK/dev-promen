...
class ContactInfoBlocksAccessibility {
    ...
    init() {
        ...
        if (typeof PromenAccessibility !== 'undefined') {
            PromenAccessibility.setupSkipLink(this.container, this.getString('skipToContent'));
        }
    }
    ...
    bindEvents() {
        ...
        this.container.addEventListener('keydown', (e) => {
            if (e.key === 'Escape') {
                ...
                this.announce(this.getString('navigationCancelled'));
            }
        });
    }
    ...
    handleLinkClick(event) {
        ...
        if (href) {
            if (href.startsWith('tel:')) {
                ...
                const callingMsg = this.getString('callingNumber', phoneNumber);
                ...
                const email = link.querySelector('[aria-hidden="true"]')?.textContent.trim()
                    || link.textContent.trim();
                const emailMsg = this.getString('composingEmail', email);
    ...
    getBlockTypeLabel(blockType) {
        ...
        return labels[blockType] || this.getString('contactInfo');
    }
    ...
    setupReducedMotion() {
        ...
        const applyReducedMotion = (matches) => {
            if (matches) {
                ...
                this.announce(this.getString('animationsDisabled'));
            } else {
