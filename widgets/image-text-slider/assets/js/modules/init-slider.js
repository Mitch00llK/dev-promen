 ...
 * 
 * Initializes the slider with the provided ID or element and options
 * @param {string|HTMLElement} sliderIdOrElement - The slider ID string or HTMLElement
 */
function initImageTextSlider(sliderIdOrElement) {
    let slider, sliderId;

    // Handle both ID string and element parameters
    if (typeof sliderIdOrElement === 'string') {
        sliderId = sliderIdOrElement;
        slider = document.getElementById(sliderId);
    } else if (sliderIdOrElement && sliderIdOrElement.nodeType === Node.ELEMENT_NODE) {
        slider = sliderIdOrElement;
        sliderId = slider.id || 'slider-' + Date.now();
        slider.id = sliderId; // Ensure element has an ID
    } else {
        return;
    }
    ...
    if (typeof Swiper === 'undefined') {
        ...
        setTimeout(function() {
            initImageTextSlider(sliderIdOrElement);
        }, 500);
    ...
    slider.swiper = imageSwiper;
    slider.imageSwiper = imageSwiper; // Keep both for compatibility
